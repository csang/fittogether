class Fitbit::BodyMeasurements < Fitbit::Data
  attr_accessor :bicep, :bmi, :calf, :chest, :fat, :forearm, :hips, :neck, :thigh, :waist, :weight, :weightgoal, :fatgoal
  ...
  def initialize(user)
    ...
    if @logged_in && @linked
      ...
      data = user.fitbit_data.body_measurements_on_date('today')
      #abort(data['goals'].inspect)
      ...
      if data['goals'].present?
        @weightgoal = data['goals']['weight']
        @fatgoal = data['goals']['fat'] unless data['goals']['fat'] == 0
      end
